...
* [cite_start]**Real-time Disease Detection:** Utilizes a pre-trained TFLite model to classify potato leaves as **Healthy**, **Early Blight**, or **Late Blight**[cite: 182, 183].
* **Tello Drone Integration:** Seamlessly launches the Tello app to fly and capture images. [cite_start]Automatically imports, compresses, and analyzes new images upon returning to SPOTATO[cite: 524, 499].
* [cite_start]**Image Handling & Compression:** Compresses all imported images (from gallery or drone) to a maximum of 1080p and 85% quality, and converts them to JPEG for consistent, efficient processing[cite: 424, 425].
* [cite_start]**Local Data Persistence:** Stores all analysis results, including the image file path, prediction, confidence, and row-tag metadata, in a local **SQLite database**[cite: 216, 219].
* [cite_start]**Album and Location Tagging:** Allows users to save a "Current Scan" session as a persistent **Album**, providing the ability to tag the results with a specific **Row Tag** (e.g., 'Row 5', 'North Field') for easy location tracking[cite: 555, 584].
* [cite_start]**Intuitive UI/UX:** Uses a clean, Poppins-based design with distinct color-coding (Dark Brown, Orange) and a **`RowDetailPage`** for managing active scan sessions[cite: 27, 484, 485].
...
### Mobile Device
* [cite_start]**OS:** Android (The Tello app and file system access features, particularly `/storage/emulated/0/Pictures/TelloPhoto`, suggest a focus on the Android platform)[cite: 413].
* [cite_start]**Required Apps:** The official **Ryze Tello** application must be installed on the device for drone connectivity[cite: 506].

### Development
* [cite_start]**Flutter:** Stable Channel (The project uses modern Flutter features like `WidgetStateProperty` and TFLite for Flutter)[cite: 276, 470].
* [cite_start]**TFLite Model:** The model asset `assets/models/model.tflite` must be present[cite: 179].
...
| :--- | :--- | :--- |
| `lib/main.dart` | Application entry point. [cite_start]Loads the TFLite model and initializes the root `SPOTATOApp`[cite: 464, 467]. | `main()` |
| `lib/config.dart` | [cite_start]Global constants, TFLite configuration, and the core **`DetectionResult`** data class[cite: 179, 186]. | `kModelAssetPath`, `DetectionResult` |
| `lib/database_helper.dart` | Manages the **SQLite database**. [cite_start]Includes functions for inserting analysis results and querying albums[cite: 210, 224]. | `tableAnalyses`, `getAllAlbums()` |
| `lib/analysis.dart` | Contains the logic for running the TFLite inference. [cite_start]**Crucially, it includes the fix to remove normalization for raw pixel values**. | `_imageToTensor()`, `runModelAnalysis()` |
| `lib/row_detail.dart` | The **Current Scan** session manager. [cite_start]Handles image picking, Tello import logic, and the Save Album dialog[cite: 485, 549, 582]. | `_importFromTelloFolder()`, `_saveCurrentScan()` |
| `lib/album_page.dart` | Displays the list of all saved albums and manages the multi-select/delete functionality[cite: 48, 53]. | `_deleteSelected()`, `build()` |
...
1.  [cite_start]**Detect Disease:** Tap the **'Detect Disease'** button[cite: 361].
2.  [cite_start]**Start Scan Session:** The user is taken to the **'Current Scan'** page (`RowDetailPage`)[cite: 485].
3.  **Add Images:** Use the **Floating Action Button (`+`)** to:
    * **Launch Tello:** Opens the Tello app. [cite_start]Upon returning, new photos will be automatically imported, compressed, and analyzed[cite: 524, 499].
    * [cite_start]**Add from Gallery:** Imports a single image from the device's gallery[cite: 514].
4.  [cite_start]**Analysis:** The image is immediately processed by the local TFLite model[cite: 545]. [cite_start]Notifications are sent when analysis is complete[cite: 519, 540].
5.  [cite_start]**Save Album:** Tap the **'Save as Album'** icon, select or enter a **Row Tag**, and save the entire scan session to a new album[cite: 622, 583, 585].
6.  [cite_start]**View Results:** Access saved scans via the **'Albums'** tab[cite: 399].
...
[cite_start]Represents the analysis data for a single image, used throughout the app (UI, Analysis, Database)[cite: 184].

| Field | Type | Description |
| :--- | :--- | :--- |
| `file` | `File` | [cite_start]The path to the processed image file on the device[cite: 186]. |
| `label` | `String?` | [cite_start]The prediction: 'Healthy', 'Early Blight', 'Late Blight', or 'Unknown'[cite: 187, 107]. |
| `confidence` | `double` | [cite_start]The confidence score of the prediction (e.g., 0.95)[cite: 188]. |
| `analysisDuration` | `Duration?` | [cite_start]The time taken for TFLite inference[cite: 188]. |
| `captureTime` | `DateTime?` | [cite_start]When the image was captured[cite: 189]. |
| `rowTag` | `String?` | [cite_start]The user-assigned location/row identifier[cite: 189]. |

### TFLite Model Constants (`config.dart`)
[cite_start]Global settings for the AI model[cite: 179].

| Constant | Value | Description |
| :--- | :--- | :--- |
| `kModelAssetPath` | `'assets/models/model.tflite'` | [cite_start]Asset path for the model[cite: 179]. |
| `kModelInputSize` | `300` | [cite_start]Expected input size (300x300 pixels)[cite: 180]. |
| `kConfidenceThreshold` | `0.35` | [cite_start]Minimum confidence to accept a prediction[cite: 181]. |
| `kModelLabels` | `List<String>` | [cite_start]The raw class names from the model output[cite: 182]. |

### Core Utilities
#### 1. Image Preprocessing (`analysis.dart`)
The core function for preparing the image tensor for the model:
